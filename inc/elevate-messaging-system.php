 ...
 */
 ...
 class elevate_messaging{
	...
	*/
	 ...
	 * 
	 * Provides the user with a notification outlining the response to their topic, sent via email
	 */
	public function send_topic_reply_email($topic_id, $reply_id){
		
		//prepare a response to determine success
		$response = array();
		$response['status'] = 'success';
		$response['message'] = 'Email sent after topic reply by ajax';
		...
		$reply = get_post($reply_id);
		$reply_author = get_user_by('ID', $reply->post_author);
		
		//execute only if we have reply author and topic author
		if(($topic_author instanceof WP_User) && ($reply_author instanceof WP_User)){
			
			//topic author info
			$topic_author_email = $topic_author->data->user_email; 			
			$topic_author_name = $topic_author->data->display_name;
			$topic_title = $topic->post_title;
			$topic_permalink = get_permalink($topic->ID); 
			
			//topic reply info
			$reply_author_name = $reply_author->data->display_name;
			$reply_author_image = get_user_meta($reply_author->ID, 'rcp_user_profile_image', true);
		
			if(!empty($reply_author_image)){
				$reply_author_image = wp_get_attachment_image_src($reply_author_image, 'thumbnail', false)[0];
			}
			$reply_date = get_the_date('l, d/m/Y, h:i:a:s', $reply_id);
			$reply_content = $reply->post_content; 
			if(!empty($reply_content)){
				$reply_content = wp_trim_words($reply_content, '25', '...');
			}
			...
			$subject = 'Healthy Imaging - Topic Reply - ' . $reply_date; 
			$headers = array(
			...
			$message .= '<article>';
				...
				$message .= '<section class="content-wrap" style="text-align: center;">';
					$message .= '<div style="width: 100%; max-width: 760px; text-align: left; display: inline-block;">';
						...
						$message .= '<div style="margin-bottom: 10px;">Hi ' . $topic_author_name . '</div>';
						$message .= '<div style="margin-bottom: 10px;">Someone has replied to your topic: <strong>' . $topic_title . '</strong></div>';
						$message .= '<div style="margin-bottom: 10px;">A summary of the reply is listed below </div>';
					$message .= '</div>';
				$message .= '</section>';
				...
				$message .= '<section class="topic-reply" style="text-align: center;">';
					...
					$message .= '<div style="background-color: #fffbea; border-left: solid 5px #ffdd00; padding: 15px; width: 100%; text-align: left; max-width: 760px; display: inline-block; margin: 15px 0px;">';
						...
						if(!empty($reply_author_image)){
							$message .= '<div>';
								$message .= '<img src="' . $reply_author_image . '" style="width: 30px; height: 30px; border-radius: 50%;"/>';
							$message .= '</div>';
						}
						$message .= '<p><b>User:</b> ' . $reply_author_name . '</p>';
						$message .= '<p><b>Date:</b> ' . $reply_date . '</p>';
						
						$message .= '<div class="message-wrap">';
							if(!empty($reply_author_image)){
								$html .= '<img src="' . $reply_author_image . '"/>';
							}
							$message .= '<div class="message" style="margin-top: 15px;">' . $reply_content . '</div>';
						$message .= '</div>';
					$message .= '</div>';
				$message .= '</section>';
			...
			if(!$mail_success){
				$response['status'] = 'error';
				$response['message'] = 'Email could not be sent out to topic author';
			}
			
		}
		//reply and topic not set
		else{
			$response['status'] = 'error';
			$response['message'] = 'Reply and or topic not set when sending email';	
		}
